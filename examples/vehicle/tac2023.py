...
def run_and_plot (ax, title) :
    ...
    ax.text(0,8,f'runtime: ${avg_runtime:.3f}\pm{std_runtime:.3f}$',fontsize=18,verticalalignment='top')
    ax.set_title(title)
...
fig, axs = plt.subplots(2,3,dpi=100,figsize=[12,8],squeeze=False)
fig.subplots_adjust(left=0.045, right=0.975, bottom=0.075, top=0.925, wspace=0.125, hspace=0.25)

# Global Interconnection Mode
clsys.incl_opts = NNCSystem.InclOpts('interconnect')
clsys.control.mode = 'global'
run_and_plot(axs[0,0], f'Input-Output Approach: $\\mathsf{{F}}^\\mathrm{{io}}$')
clsys.control.mode = 'hybrid'
axs[0,0].set_ylabel('$p_y$',labelpad=6, rotation='horizontal')

# Hybrid Interconnection Mode
clsys.incl_opts = NNCSystem.InclOpts('interconnect')
run_and_plot(axs[1,0], f'Interconnection: $\\mathsf{{F}}^\\mathrm{{con}}$')
axs[1,0].set_xlabel('$p_x$',labelpad=2); 
axs[1,0].set_ylabel('$p_y$',labelpad=6, rotation='horizontal')

# Cornered Mode
clsys.incl_opts = NNCSystem.InclOpts('jacobian')
clsys.set_two_corners()
run_and_plot(axs[0,1], f'Cornered: $\\mathsf{{F}}^\\mathrm{{act}}$')

# Cornered+Interconnection Mode
clsys.incl_opts = NNCSystem.InclOpts('jacobian+interconnect')
clsys.set_two_corners()
run_and_plot(axs[1,1], f'Intersection: $\\mathsf{{F}}^\\mathrm{{con}}\\wedge \\mathsf{{F}}^\\mathrm{{act}}$')
axs[1,1].set_xlabel('$p_x$',labelpad=2); 

# Mixed States Cornered Mode
clsys.incl_opts = NNCSystem.InclOpts('jacobian', orderings=[Ordering((0,1,2,3))])
clsys.set_two_corners()
run_and_plot(axs[0,2], f'Mixed-State Cornered: $\\mathsf{{F}}^\\mathrm{{con}}$')
...
run_and_plot(axs[1,2], f'Mixed-All Cornered: $\\mathsf{{F}}^\\mathrm{{con}}$')
axs[1,2].set_xlabel('$p_x$',labelpad=2); 
